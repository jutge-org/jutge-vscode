...
**Activation events** include `onStartupFinished`, `onWebviewPanel:problemView`, and `onView:…` for each contributed view so the sidebar and restored problem panels load reliably—not only on first webview use.
...
- **`jutgeStatsView`**: `TreeView` for the unsigned **Jutge Stats** view.
...
| Jutge Stats       | **`JutgeStatsTreeProvider`** (`jutge-stats-view/provider.ts`)          | Public homepage stats when not signed in                   |
...
| Timer             | **`TimerWebviewViewProvider`** (`timer-view/provider.ts`)              | Webview view; countdown / exam time                        |
...
- **`WebviewPanelRegistry`** (`providers/problem-view/panel-registry.ts`)
- **`ProblemViewPanel`** (`panel.ts`) + **`htmlProblemView`** (`html.ts`) + **`ProblemHandler`**
- **`ProblemViewPanelSerializer`** (`panel-serializer.ts`)
...
| `coursesView`, `jutgeStatsView`          | `extension.ts`      | Tree views                                    |
...
| `JutgeStatsTreeProvider`               | `providers/jutge-stats-view/provider.ts`     | Jutge stats                    |
...
| `TimerWebviewViewProvider`             | `providers/timer-view/provider.ts`           | Timer webview                  |
| `ProblemViewPanel`                     | `providers/problem-view/panel.ts`            | Problem panel + messaging      |
| `WebviewPanelRegistry`                 | `providers/problem-view/panel-registry.ts`   | Panel lifecycle                |
| `ProblemViewPanelSerializer`           | `providers/problem-view/panel-serializer.ts` | Restore state                  |
...
src/
  ...
  providers/
    ...
    exam-view/
    jutge-stats-view/
    problem-view/           # panel, HTML, registry, serializer
    profile-view/
    ranking-view/
    sign-in-view/
  services/
...
3. `src/providers/problem-view/panel.ts` + `src/services/problem-handler.ts` — problem workflow end-to-end.
