...
export class JutgeApiClient {
    ...
    async execute(
        ...
        const { output, error, duration, operation_id, time } = JSON.parse(
            odata as string
        )
...
class Module_admin_stats {
     ...
     *
     */
    async getDistributionOfUsersBySubmissions(
        data: number
