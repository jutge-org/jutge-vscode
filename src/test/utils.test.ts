...
suite("Utils Test Suite", () => {
    ...
    test("chooseFromEditorList should return the only editor for a list of one", async () => {
        ...
        const editor = await vscode.window.showTextDocument(vscode.Uri.joinPath(workspaceFolder.uri, "test_file.cc"))
    ...
    test("getDefaultProblemId should return correct problem id for known problem", async function () {
        await vscode.workspace.getConfiguration("jutge-vscode").update("problem.preferredLang", "Català")
        assert.equal(utils.getDefaultProblemId("P68688"), "P68688_ca")

        await vscode.workspace.getConfiguration("jutge-vscode").update("problem.preferredLang", "Castellano")
        assert.equal(utils.getDefaultProblemId("P68688"), "P68688_es")

        await vscode.workspace.getConfiguration("jutge-vscode").update("problem.preferredLang", "English")
