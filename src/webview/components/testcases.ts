...
function generateTestcase(testcase: Testcase, index: number): string {
...
export function generateTestcases(problemTestcases: Testcase[], handler: string | null): string {
    if (handler !== "std") {
        return /*html*/ `
    ...
    return /*html*/ `
        ...
        <div class="testcase-panels">
            ${problemTestcases.map(generateTestcase).join("")}
        </div>
    `
}
